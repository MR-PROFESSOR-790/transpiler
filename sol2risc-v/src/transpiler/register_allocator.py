...
class RegisterAllocator:
    ...
    def allocate(self):
        with self.lock:
            ...
            self.spill_area.append(None)  
            logger.warning(f"No available registers. Spilled to memory slot {spill_slot}.")
    ...
    def free(self, reg):
        with self.lock:
            if reg in self.allocated_registers:
                ...
                logger.warning(f"Tried to free unallocated register or invalid spill slot: {reg}")

    def reset(self):
        with self.lock:
            self.allocated_registers.clear()
            self.spill_area.clear()
            logger.debug("Reset all allocated registers and spill slots.")

    def is_allocated(self, reg):
        return reg in self.allocated_registers

    def list_allocated(self):
        return list(self.allocated_registers) + [f"spill[{i}]" for i, slot in enumerate(self.spill_area) if slot is not None]